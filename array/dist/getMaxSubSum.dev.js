...
function get_max_sub_sum(arr) {
  var sum = tempSum = arr[0] > 0 ? arr[0] : 0;
...
function getMaxSubSum(arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    for (var j = i; j < arr.length - 1; j++) {
      console.log("".concat(arr[j] + 1, " === ").concat(arr[i + y], " ")); // if arr[0] + 1 === arr[0 + 1]   ->   if 1 + 1 === 2
...
console.log(getMaxSubSum([1, 2, 3, 4])); // console.log(getMaxSubSum([7,8]))
